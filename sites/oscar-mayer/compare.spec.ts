...
// palette (Yellow, Red, Cream, etc.). analyzePage() will run the compliance
// check against every set and report the violations from whichever one
// matches the page most closely.
//
// tokensDir is read from sites/oscar-mayer/site.config.json so the location
// can be changed without touching this spec; the path is resolved relative to
// the repo root (process.cwd() when Playwright runs).
