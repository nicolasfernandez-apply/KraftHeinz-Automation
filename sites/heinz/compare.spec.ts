...
const pagesPath = path.resolve(process.cwd(), 'sites', 'heinz', 'pages.json');

if (!fs.existsSync(pagesPath)) {
  throw new Error(
    '\nNo pages.json found for Heinz.\n' +
    'Run the crawler first:  npm run crawl:heinz\n',
  );
}

const { comparisons: allUrlPairs }: { comparisons: UrlPair[] } = JSON.parse(
  fs.readFileSync(pagesPath, 'utf8'),
);

if (!Array.isArray(allUrlPairs) || allUrlPairs.length === 0) {
  throw new Error('pages.json is empty or malformed. Re-run: npm run crawl:heinz');
}

// ── Locale filtering ─────────────────────────────────────────────────────────
// LOCALE env var selects which locale(s) to compare:
//   unset / "all"  → every page
//   "default"      → only pages without a /xx-XX/ prefix
//   "es-MX" etc.   → only pages under that locale prefix
// Run `npm run locales:heinz` to see all available locales.

const LOCALE_RE = /^https?:\/\/[^/]+\/([a-z]{2}-[A-Z]{2})(\/|$)/;

function localeOf(pair: UrlPair): string {
  const match = pair.productionUrl.match(LOCALE_RE);
  return match ? match[1] : 'default';
}

const localeFilter = (process.env.LOCALE ?? 'all').trim();
const urlPairs: UrlPair[] =
  localeFilter === '' || localeFilter.toLowerCase() === 'all'
    ? allUrlPairs
    : allUrlPairs.filter((p) => localeOf(p) === localeFilter);

if (urlPairs.length === 0) {
  throw new Error(
    `No pages matched LOCALE="${localeFilter}". Run \`npm run locales:heinz\` to see available locales.`,
  );
}

console.log(
  `[compare] LOCALE="${localeFilter}" — ${urlPairs.length} of ${allUrlPairs.length} page(s) selected.`,
);
...
test.describe('Heinz — Preview vs Production', () => {
  for (const pair of urlPairs) {
    test(pair.name, async ({ browser }, testInfo) => {
      const auth = requireAuthConfig();

      const reportsDir     = path.join(process.cwd(), 'sites', 'heinz', 'reports');
